...
struct MonsterView: View {
  ...
  var body: some View {
    Group {
      if let monster = monsterModel {
        VStack {
          InfoPlate(monster)
            MonsterSpriteView(evolutionIndex: monster.experienceComponent.evolutionIndex)
            .padding()
            .onTapGesture {
              monster.pet()
            }
          EnergyView(energy: isCatchingUp ? displayedEnergy : monster.energy)
          Button("Feed") {
            monster.feed()
          }
  ...
  func InfoPlate(_ monster: MonsterModel) -> some View {
    let exp = isCatchingUp ? displayedExp : monster.experienceComponent.exp
      let expCap = isCatchingUp ? displayedExpCap : monster.experienceComponent.expCap
